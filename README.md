...
앱 설치 없이 브라우저만으로 동작하는 단일 HTML. ES 모듈·카메라를 쓰므로 `file://`가 아니라 **로컬 서버**로 띄워야 합니다.

```bash
cd poeun-ar
npm run dev        # → http://localhost:5173  (npx serve)
# 또는
npm run dev:py     # python3 http.server 사용
```

- `localhost`는 보안 컨텍스트라 데스크탑에서 카메라 권한 동작.
- **실기기(휴대폰) 테스트는 HTTPS 필수** — 카메라·GPS가 http에서 차단됨. ngrok 등으로 https 터널 또는 https 호스팅 사용.
- API 키 없이도 **데모 모드**로 전체 흐름(소환·대화 폴백·TTS·지오펜스)이 돕니다.

## 📂 구조

```
poeun-ar/
├── index.html              # 작업 진입점 (프로토타입에서 확장해 나감)
...
| 우선순위 | 기능 | 비고 |
|---|---|---|
| **P1** | 비주얼 퀘스트(사진 미션) | 핵심 차별점, 최우선. `missions.json` 기반 |
| **P2** | AI 비전 사진 판정 | Claude vision으로 자유 미션 판정, 호출 상한 |
| **P3** | 백엔드 서버 | API 키 보호 + 위치 서버 재검증 (`server/`) |
